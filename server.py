...
# shared session reuses tcp connections for faster fetches
...
def fetch_tiles_parallel(tileset, zoom, base_x, base_y, grid):
    # fetch all tiles concurrently instead of sequentially
    jobs = []
...
def fetch_and_stitch_terrain_with_overlap(zoom, x, y):
    # fetch a 3x3 grid of terrain tiles centered on our target
    # this gives us 1 tile of overlap on every side for seamless neighbor matching
    ...
    # fetch our tile + 1 tile padding on each side
    # only needs edge tiles for seam fix, not full 3x3
    # here we just fetch full grid for simplicity, then sample edges from neighbors
...
def get_terrain(lat, lon, zoom):
    ...
    heights = fetch_and_stitch_terrain_with_overlap(zoom, x, y)
